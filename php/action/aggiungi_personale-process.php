<?php
  ...
  if($res && $res->num_rows){
    ...
    $query = "SELECT ID, Qualifica FROM Dispone WHERE ID_Persona=(
      ...
      ID_Palestra=".$p_id_palestra;
    $res = $connection->query($query);
    if($res->num_rows){
      $row = $res->fetch_assoc();
      if($p_qualifica>$row['Qualifica']){
        send_message($page, $p_nome." ".$p_cognome." dispone di un grado più alto, nessuna promozione effettuato.");
      }elseif($p_qualifica=$row['Qualifica']){
        send_message($page, $p_nome." ".$p_cognome." dispone dello stesso grado, nessuna promozione effettuato.");
      }

      //controlliamo se il grando è tale da poter poter effettuare la Promozione
      check_qualifica($connection, $p_id_palestra, $p_qualifica, $page);

      $query = "UPDATE Dispone SET Qualifica=".$p_qualifica." WHERE
        ID=".$row['ID'];
      $connection->query($query);
      if($connection->error){
        echo "Mysql error: ".$connection_error;
        $connection->close();
        die();
      }
      send_message($page, "L\'utente era già registrato, l\'ho promosso in grado!");
    }else{
      //se non è una promozione ma un assunzione
      check_qualifica($connection, $p_id_palestra, $p_qualifica, $page);
      ...
      $query = "INSERT INTO Dispone(ID_Persona, ID_Palestra, Qualifica) values(
        ".$row['ID_Persona'].",".$p_id_palestra.",".$p_qualifica.")";
      $connection->query($query);
      if($connection->error){
      ...
      send_message($page, "Assunzione avvenuta con successo!");
    }
  }else{
    ...
    $query = "INSERT INTO Dispone(ID_Persona, ID_Palestra, Qualifica) values(
      ".$connection->insert_id.",".$p_id_palestra.",".$p_qualifica.")";
    $connection->query($query);
    if($connection->error){
      echo "Mysql error: ".$connection->error;
      $connection->close();
